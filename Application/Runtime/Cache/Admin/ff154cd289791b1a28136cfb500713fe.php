	...
	<table class="table table-hover table-striped">
		...
		<tbody>
			<?php if(is_array($aritclescrap)): foreach($aritclescrap as $key=>$v): ?><tr>
					<td><?php echo ($v["id"]); ?></td>
					<td><a href="<?php echo U('show/index?id='); echo ($v["id"]); ?>" style="color: inherit;"><?php echo ($v["title"]); ?></a></td>
					<td>
					...
					<?php if($v["status"] == 1): ?><td><a href="<?php echo U('articlescrap/post?id='); echo ($v["id"]); ?>" style="color:red;">撤销</a> | <a href="<?php echo U('articlescrap/update?id='); echo ($v["id"]); ?>">编辑</a> | <a href="<?php echo U('articlescrap/delete?id='); echo ($v["id"]); ?>" style="color:red;" onclick="javascript:return del('您真的确定要删除吗？\n\n删除后将不能恢复!');">删除</a></td>
						<?php else: ?>
						<td><a href="<?php echo U('articlescrap/post?id='); echo ($v["id"]); ?>" style="color:#50AD1E;">发布</a> | <a href="<?php echo U('articlescrap/update?id='); echo ($v["id"]); ?>">编辑</a> | <a href="<?php echo U('articlescrap/delete?id='); echo ($v["id"]); ?>" style="color:red;" onclick="javascript:return del('您真的确定要删除吗？\n\n删除后将不能恢复!');">删除</a></td><?php endif; ?>
