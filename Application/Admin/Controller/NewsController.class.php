...
class NewsController extends BaseController {
	...
	public function update($id) {
		//默认显示添加表单
		if (!IS_POST) {
			$model = M('News') -> where("id= %d",$id) -> find();
	...
	public function post($id) {//post到前台
		if (IS_GET) {
			$status = M('News') -> where("id= %d",$id) -> getField('status');
			if ($status === '0') {
				$data['status'] = 1;
			} else {
				$data['status'] = 0;
			}
			$result = M('News') -> where("id= %d",$id) -> save($data);
